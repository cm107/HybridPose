...
def parse_args():
    ...
    parser.add_argument('--fx', type=float, default=700.) # Make sure that these intrinsic camera parameters match up with cfg.render_K in pvnet-rendering/config.py
    ...
    parser.add_argument('--dst_label_dir', type=str, required=True)
    parser.add_argument('--ply_path', type=str, required=True)
    parser.add_argument('--ply_path_before', type=str, default=None)
    parser.add_argument('--symmetries_path', type=str, required=True)
    parser.add_argument('--keypoints_3d_path', type=str, required=True)
    parser.add_argument('--sample_save_path', type=str, default=None)
...
def main(args):
    ...
    num_examples = get_num_examples(args.src_data_dir)
    ...
    for example_id in range(num_examples):
        ...
        R, T = read_pose(args.src_data_dir, example_id)
        ...
        sample = example_id == 0
        if sample:
            img = cv2.imread('{}/{}.jpg'.format(args.src_data_dir, example_id))
        ...
        for P_idx, P in enumerate(P_list):
            ...
            if y >= 0 and y < args.img_h and x >= 0 and x < args.img_w:
                if is_filled[y, x] == 0 or z_buffer[y, x] > z:
                    ...
                    mask[y, x] = 255
        mask = fill(mask)
        cv2.imwrite(f'{args.dst_label_dir}/mask{example_id}.png', mask)
        ...
        np.save(f'{args.dst_label_dir}/cor{example_id}.npy', correspondences)
        if sample and args.sample_save_path is not None:
            cv2.imwrite(args.sample_save_path, img)
    ...
    np.save(f'{args.dst_label_dir}/keypoints_2d.npy', keypts_2d)
