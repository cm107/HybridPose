...
class BlenderLinemodDataset(Dataset):
    ...
    def __init__(
        ...
        labels_dir: str, # Generated from HybridPose
        keypoints_3d_path: str, # Generated from clean-pvnet
        symmetries_path: str, # Generated from symseg
        object_name,
        size=200,
        augment=True,
        occlude=False,
        ...
        self.length = len(list(filter(lambda x: x.endswith('jpg'), os.listdir(self.data_dir))))
        self.size = size
        # pre-load data into memory
        self.pts2d = np.float32(np.load(os.path.join(self.labels_dir, 'keypoints_2d.npy')))
        ...
        self.split = split
        self.pts3d = np.float32(np.load(keypoints_3d_path))
        self.normal = self.read_normal(symmetries_path)
    ...
    def __getitem__(self, idx):
        ...
        # image
        image_name = os.path.join(self.data_dir,
                                  '{}.jpg'.format(idx))
        image = cv2.imread(image_name)
        # mask
        mask_name = os.path.join(self.labels_dir,
                                 'mask{}.png'.format(idx))
        ...
        sym_cor_name = os.path.join(self.labels_dir,
                                    'cor{}.npy'.format(idx))
